		...
		if (IsAlive) {
			...
			if (waterDigesting > 0) {
				...
				if (vitalBodyWater < maxVitalBodyWater) { // Does the body need water?
					vitalBodyWater += waterDigestionRate * Time.fixedDeltaTime * metabolicSpeedScale; // Put the water into bodily circulation.
				} else {
					bladderContents += waterDigestionRate * Time.fixedDeltaTime * metabolicSpeedScale; // Put the water into the bladder; 
				}
	...
	public IngestionResult Ingest(ConsumableObject ingestedObject) {
		if (isAcceptingFood) {
			if (ingestedObject.ObjectVolume < (stomachCapacity - DigestiveCapacityOccupied)) {
				...
				// There is insufficent digestive capacity for the object
				if (ingestedObject.isHalfable) {
